...
"""
sedit ― シンプルな Tkinter エディタ (Windows 向け)
"""
import os
import sys
import json
...
# ----------------------------------------------------------------------
def _get_log_path():
    """ログファイルへのパスを返す。 Windows では %LOCALAPPDATA% を使用。 """
...
def _get_settings_path():
    """設定ファイルへのパスを返す。 Windows では %APPDATA% を使用。 """
    if sys.platform.startswith('win'):
        base = os.path.join(
...
menu_ext.add_command(label='拡張機能フォルダを開く',
                     command=lambda: open_path(EXT_DIR))
