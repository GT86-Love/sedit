...
"""
sedit ― シンプルな Tkinter エディタ (macOS 用)
"""
import os
import sys
import json
...
def _get_log_path():
    ...
    return os.path.join(base, 'sedit.log')
...
def _get_settings_path():
    """設定ファイルのパスを返す。"""
    base = os.path.join(os.path.expanduser('~'), 'Library', 'Preferences', 'sedit')
    os.makedirs(base, exist_ok=True)
...
def open_log_viewer():
    ...
    ttk.Button(btnf, text='Open Log File',
               command=lambda: subprocess.Popen(['open', LOG_PATH],
...
menu_ext.add_command(label='拡張機能フォルダを開く',
                     command=lambda: subprocess.Popen(['open', EXT_DIR],
                                                stdout=subprocess.DEVNULL,
